...
% Init position
pos_all = seq.init_rect(:,[2,1]);
UAV_num=size(pos_all,1);
...
projection_matrix = cell(1,UAV_num,2);
% Initialize and allocate
prior_weights_all=cell(1,UAV_num);
for UAV_cnt=1:UAV_num
    prior_weights_all{1,UAV_cnt} = zeros(params.nSamples,1, 'single');
end
% sample_weights = cast(prior_weights, 'like', params.data_type);
samplesf_all = cell(1, UAV_num, num_feature_blocks);
for UAV_cnt=1:UAV_num
    for k = 1:num_feature_blocks
        samplesf_all{1,UAV_cnt, k} = zeros(params.nSamples,sample_dim(k),filter_sz(k,1),(filter_sz(k,2)+1)/2, 'like', params.data_type_complex);
    end
end
...
distance_matrix_all=cell(1,UAV_num);
for UAV_cnt=1:UAV_num
    distance_matrix_all{1,UAV_cnt} = inf(params.nSamples, 'single');
end

% Kernel matrix, used to update distance matrix
gram_matrix_all=cell(1,UAV_num);
for UAV_cnt=1:UAV_num
    gram_matrix_all{1,UAV_cnt} = inf(params.nSamples, 'single');
end

latest_ind = [];
frames_since_last_train = inf;
num_training_samples = zeros(1,UAV_num);
...
while true
    ...
    for UAV_cnt=1:UAV_num
        ...
        pos = pos_all(UAV_cnt,:);
        samplesf = samplesf_all(1,UAV_cnt,:);
        distance_matrix = distance_matrix_all{1,UAV_cnt};
        gram_matrix = gram_matrix_all{1,UAV_cnt};
        prior_weights = prior_weights_all{1,UAV_cnt};
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if seq.frame > 1
            ...
            % Project sample
            xt_proj = project_sample(xt, projection_matrix(1,UAV_cnt,:));
        ...
        if seq.frame == 1
            ...
            projection_matrix(1,UAV_cnt,:) = init_projection_matrix(xl, sample_dim, params);

            % Project sample
            xlf_proj = project_sample(xlf, projection_matrix(1,UAV_cnt,:));
            
            clear xlw
        elseif params.learning_rate > 0 && model_update_flag==1
            if ~params.use_detection_sample
                ...
                % Project sample
                xl_proj = project_sample(xl, projection_matrix(1,UAV_cnt,:));
        ...
        if params.use_sample_merge
            ...
            [merged_sample, new_sample, merged_sample_id, new_sample_id, distance_matrix, gram_matrix, prior_weights] = ...
                ...
                num_training_samples(1,UAV_cnt),params);
            if num_training_samples(1,UAV_cnt) < params.nSamples
                num_training_samples(1,UAV_cnt) = num_training_samples(1,UAV_cnt) + 1;
            end
        end
        ...
        if train_tracker     
            ...
            if seq.frame == 1
                ...
                if params.update_projection_matrix
                    ...
                    proj_energy = cellfun(@(P, yf) 2*sum(abs(yf(:)).^2) / sum(feature_dim) * ones(size(P), 'like', params.data_type), projection_matrix(1,UAV_cnt,:), yf, 'uniformoutput', false);
                else
            ...
            if seq.frame == 1 && params.update_projection_matrix
                if params.debug
                    projection_matrix_init = projection_matrix(1,UAV_cnt,:);
                end
                
                % Initial Gauss-Newton optimization of the filter and
                % projection matrix.
                [hf, projection_matrix(1,UAV_cnt,:), res_norms] = train_joint(hf, projection_matrix(1,UAV_cnt,:), xlf, yf, reg_filter, sample_energy, reg_energy, proj_energy, params, init_CG_opts);
                ...
                if params.debug
                    ...
                    norm_proj_mat = sqrt(sum(cellfun(@(P) gather(norm(P(:))^2), projection_matrix(1,UAV_cnt,:))));
                    norm_proj_mat_change = sqrt(sum(cellfun(@(P,P2) gather(norm(P(:) - P2(:))^2), projection_matrix_init, projection_matrix(1,UAV_cnt,:))));
    ...
    %                 set(fig_handle, 'Position', [100, 100, 100+size(im,2), 100+size(im,1)]);
                ...
                rectangle('Position',rect_position_vis, 'EdgeColor','g', 'LineWidth',2);
    %             text(100, 10, int2str(sum(sum(sampled_scores_display))), 'color', [0 1 1]);
    %             text(200, 10, int2str(sum(sum(abs(scores_fs)))), 'color', [0 1 1]);
                text(10, 10, int2str(seq.frame), 'color', [0 1 1]);
    %             fprintf('%d    %d\n',seq.frame,sum(sum(abs(scores_fs))));
                hold off;
    ...
    %          pause
        ...
        pos_all(UAV_cnt,:)=pos;
        samplesf_all(1,UAV_cnt,:)=samplesf;
        distance_matrix_all{1,UAV_cnt}=distance_matrix;
        gram_matrix_all{1,UAV_cnt}=gram_matrix;
        prior_weights_all{1,UAV_cnt}=prior_weights;
    end
end
