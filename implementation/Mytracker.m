...
while true
    ...
    if seq.frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            sampled_scores_display = fftshift(sample_fs(scores_fs(:,:,scale_ind), output_sz));
            confidence_score=sum(sum(sampled_scores_display));
            if seq.frame==2
                % confidence init
                avg_score=confidence_score;
                % movement init
                avg_move=translation_vec;
            end
            
            % judge whether blocking   
            if (avg_score-confidence_score)/avg_score>0.5% sharp decrease==blocking
                model_update_flag=0;% stop model update
                % move by momentum
                pos = sample_pos + avg_move;
            else% not blocking
                pos = sample_pos + translation_vec;

                % confidence update
                avg_score=avg_score*0.6+confidence_score*0.4;
                % movement update
                avg_move = avg_move*0.6 + translation_vec*0.4;

                model_update_flag=1;
            end
%             fprintf('frame:%d confidence:%.2f avg_confidence:%.2f\n',seq.frame,confidence_score,avg_score);
